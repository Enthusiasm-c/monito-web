...
class EnhancedPdfProcessor:
    ...
    def process_pdf(self, pdf_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Clean up (only if it's a downloaded temp file)
            if not os.path.isfile(pdf_url):
                os.unlink(temp_path)
    ...
    def download_pdf(self, url: str) -> Optional[str]:
        """Download PDF to temporary file or use local file"""
        try:
            # Check if it's a local file
            if os.path.isfile(url):
                self.log(f"✅ Using local file: {url}")
                return url
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for page_num, page in enumerate(pdf.pages):
                    ...
                    if tables:
                        ...
                        # No tables found - try text-based extraction
                        text = page.extract_text()
                        if text:
                            text_products = self.extract_products_from_text(text, page_num + 1)
                            if text_products:
                                self.log(f"  📝 Extracted {len(text_products)} products from text")
                                all_products.extend(text_products)
                                total_rows += len(text_products)
    ...
    def process_table_dataframe(self, df: pd.DataFrame, source: str) -> List[Dict[str, Any]]:
        ...
        # Check for complex multi-price tables (like Island Organics)
        if self.is_complex_price_table(df):
            self.log(f"🔍 Detected complex multi-price table in {source}")
            products = self.extract_products_complex_table(df, source)
            if products:
                return products
    ...
    def parse_price(self, price_str: str) -> float:
        ...
        # Pattern 3: Indonesian format with dots as thousand separator (e.g., 316.350)
        # Match numbers with dots followed by exactly 3 digits
        indo_format_match = re.search(r'(\d+(?:\.\d{3})+)', price_str)
        if indo_format_match:
            price_text = indo_format_match.group(1).replace('.', '')
            try:
                return float(price_text)
            except:
                pass
        ...
        # Handle dots as thousand separators if followed by 3 digits
        if '.' in cleaned:
            # Check if dots are used as thousand separators
            parts = cleaned.split('.')
    ...
    def extract_products_from_text(self, text: str, page_num: int) -> List[Dict[str, Any]]:
        """Extract products from text-based PDFs (like VALENTA)"""
        import re
        ...
        while i < len(lines):
            ...
            if not line or any(skip in line.upper() for skip in ['DELIVERY', 'PRICE NOT INCLUDE', 'INDONESIAN CHEESE']):
                i += 1
                continue
            ...
            if price_match:
                ...
                if product_name and len(product_name) > 3:
                    # Parse price with K suffix support
                    ...
                    if price_value > 0:
                        products.append({
                            ...
                            'category': 'cheese',
                            'description': None,
                            'sourceMethod': f'text_extraction_page_{page_num}'
                        })
    ...
    def is_complex_price_table(self, df: pd.DataFrame) -> bool:
        """Detect if table has multiple price columns (wholesale/retail)"""
        if df.empty or len(df.columns) < 4:
            return False
            
        # Look for headers indicating multiple prices
        headers = []
        if len(df) > 0:
            # Check first row for headers
            first_row = df.iloc[0]
            headers = [str(cell).lower() for cell in first_row]
            
        # Check column names
        col_names = [str(col).lower() for col in df.columns]
        all_headers = headers + col_names
        
        # Look for wholesale/retail indicators
        price_indicators = ['wholesale', 'retail', 'ws price', 'retail price', 'w/s', 'r/p']
        multi_price_count = sum(1 for header in all_headers if any(ind in header for ind in price_indicators))
        
        return multi_price_count >= 2
    
    def extract_products_complex_table(self, df: pd.DataFrame, source: str) -> List[Dict[str, Any]]:
        """Extract products from complex tables with multiple price columns"""
        products = []
        ...
        try:
            ...
            for col_idx in range(len(df.columns)):
                col_values = df.iloc[:min(20, len(df)), col_idx]
                
                # Check if column contains prices
                price_count = 0
                for val in col_values:
                    if self.parse_price(str(val)) > 0:
                        price_count += 1
                        
                if price_count >= 3:  # At least 3 valid prices
                    price_columns.append(col_idx)
                    
            self.log(f"  Found {len(price_columns)} price columns in complex table")
            
            # Extract products with each price type
            for idx, row in df.iterrows():
                try:
                    ...
                    # Extract each price as separate product
                    for price_col in price_columns:
                        if price_col < len(row):
                            ...
                            if price > 0:
                                # Determine price type from column header
                                price_type = "standard"
                                if len(df) > 0 and price_col < len(df.columns):
                                    header = str(df.columns[price_col]).lower()
                                    if 'wholesale' in header or 'ws' in header:
                                        price_type = "wholesale"
                                    elif 'retail' in header:
                                        price_type = "retail"
                                        
                                # Extract unit/size info
                                unit = self.extract_unit_from_row(row)
                                
                                product = {
                                    "name": f"{name} ({price_type})",
                                    "price": float(price),
                                    "unit": unit,
            ...
            self.log(f"  Extracted {len(products)} products from complex table")
            return products
            
        except Exception as e:
            self.log(f"Error in complex table extraction: {e}", 'ERROR')
            return products
    
    def extract_unit_from_row(self, row) -> str:
        """Extract unit information from a table row"""
        unit_keywords = {
            'kg': 'kg',
            'gram': 'gr',
            'gr': 'gr',
